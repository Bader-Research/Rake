...
DATA serial_express_eval(NODE* array,DATA vertices);

// initializes non-root nodes for a FULL Binary tree 
void internal(NODE *array, DATA positon, DATA vertices, DATA internal_nodes, 
	      int ran, THREADED);

// creates a FULL, CAT or RANDOM binary tree
void init_array(NODE *array, DATA vertices, int levels, int ran, int type, 
		THREADED);

// evaluates expression like algorithm in JaJa's Book with data copy (step 2.3)
DATA express_eval(NODE *array,int levels, DATA * leaves, NODE_PTR *A, NODE_PTR *B, 
		 THREADED);
...
DATA express_eval_nodatacopy(NODE *array,DATA leaves,int levels,NODE_PTR *A,THREADED);
 				
// RAKE OPPS 
void Rake_LC(DATA i, NODE_PTR *A, NODE *array, THREADED);
void Rake_RC(DATA i, NODE_PTR *A, NODE *array, THREADED);
				
// creates array with info about tree to pass to listranking 
void crt_edg_arry(NODE *array, list_t *edg_arry, DATA vertices, THREADED);
				
// used in testing, makes all the leaves hold a value of 1
void make_c_1(NODE *array, DATA vertices,THREADED);
				
// in tree creation->operation value.can be all 1-*, all 0 - +, or random
int crt_opp(int ran, THREADED);
...
void *SIMPLE_main(THREADED)
{
  ...
  list_t *edg_arry;
  DATA i,*leaves,vertices,serialresult,parallelresult,v=1,lrnoelems,ml;
  int levels;
  FILE *fp;
  ...
  leaves=(DATA *)node_malloc(sizeof(DATA),TH);
  ...
  on_one_thread
  {  
    fprintf(fp,"Random: %d TYPE: %d THREADS: %d Vertices: %ld levels: %d\n",ran,type,THREADS,vertices,levels);
    fflush(fp);
      }
  #endif
...
#if DEBUG0
  on_one_thread{
  fprintf(fp,"Trying %ld bytes for edg_arry allocation....\n",sizeof(list_t)*lrnoelems);
  fflush(fp);
  }
#endif

  edg_arry=(list_t *)node_malloc(sizeof(list_t)*lrnoelems,TH);

#if DEBUG0
  on_one_thread{
  fprintf(fp,"allocated edg_arry! Trying %ld bytes for array allocation\n",sizeof(NODE)*vertices);
  fflush(fp);
  }
#endif

  array=(NODE *)node_malloc(sizeof(NODE)*vertices, TH);
  node_Barrier();

#if DEBUG0
  on_one_thread{
  fprintf(fp,"Successfully allocated both arrays!!!!\n");
  fflush(fp);
  }
#endif
...
#if DEBUG0
  fprintf(fp,"be4: Thread[%d]: leaves: %d\n",MYTHREAD,*leaves);
  fflush(fp);

  node_Barrier();

  fprintf(fp,"after: Thread[%d]: leaves: %d\n",MYTHREAD,*leaves);
  fflush(fp);
#endif
  ...
  on_one_thread{
  fprintf(fp,"after lr\n");
fprintf(fp,"leaves: %d node_ptr: %d alloc: %d\n",*leaves,sizeof(NODE_PTR),(sizeof(NODE_PTR)*(*leaves)));
  fflush(fp);
  }
#endif
  ...
  A=(NODE_PTR *)node_malloc(sizeof(NODE_PTR)*(*leaves),TH);
  B=(NODE_PTR *)node_malloc(sizeof(NODE_PTR)*(*leaves),TH);
  ...
  *****************************************************************************/  
  ...
  node_partition_loop_across_threads(i,1,vertices,1) {
    if (array[i].opp==NIL)
  ...
  /*Expression Evaluation with no data copy. Not used because it is inefficient as compared to the express_eval function which has data copy */
...
#if DEBUG0
  fprintf(fp,"after rake\n");
  fflush(fp);
#endif

    if(serialresult!=parallelresult){
      fprintf(fp,"%d %d %d ERROR: Serial Result: %ld  Parallel Result: %ld\n",type,THREADS,levels,serialresult,parallelresult);
    }
    else{
...
void init_array(NODE *array, DATA vertices, int levels, 
		int ran, int type, THREADED)
{
  ...
  switch(type)
      ...
      node_partition_loop_across_threads (i, 1, vertices, 1)
	{
      ...
      node_partition_loop_across_threads (i, 1, vertices-1, 2)
	{
	  array[i].LC=1;
	  array[i+1].LC=0;
	}
...
DATA express_eval(NODE *array,int levels, DATA *leaves, NODE_PTR *A, NODE_PTR *B, 
  ...
  for (i=0; i<levels - 1; i++){	    
    if (flag){
      ...
      node_partition_loop_across_threads(j,1,*leaves-1, 2){ 
	if (A[j]->LC==TRUE)
	  Rake_LC(j, A, array, TH);
      }
      node_Barrier();
	  
      node_partition_loop_across_threads(j,1,*leaves-1, 2){ 
	if (A[j]->LC==FALSE)
	  Rake_RC(j, A, array, TH);
      } 
      node_Barrier();

      newleaves=((*leaves-2)>>1)+1;	  
      // copy the non-raked leaves from A to B array. first and last leaf special
      node_partition_loop_across_threads(j,1,newleaves, 1){
	B[j]=A[(j<<1)];
      }
      node_Barrier();
	...
	*leaves=newleaves+1;
    ...
    if (!flag){
      ...
      node_partition_loop_across_threads(j,1,*leaves-1, 2){ 
	if (B[j]->LC==TRUE)
	  Rake_LC(j, B, array, TH);
      }	  
      node_Barrier();
	
      node_partition_loop_across_threads(j,1,*leaves-1, 2){ 
	if (B[j]->LC==FALSE)
	  Rake_RC(j, B, array, TH);
      }	  
      node_Barrier();
  ...
  if (n1->opp==MULT)
    ...
    on_one_thread
      {
	fprintf(outfile,"\nParallel result: The value of expression is %ld\n ", final_value);
	fflush(outfile);
      }
#endif
...
DATA express_eval_nodatacopy(NODE *array,DATA leaves,int levels,NODE_PTR *A,THREADED){
  ...
  if (n1->opp==MULT)
    ...
    on_one_thread
      {
        fprintf(outfile,"\nexpress_eval_nodatacopty: Parallel result: The value of expression is %ld\n ",final_value);
        fflush(outfile);
      }
#endif
...
int crt_opp(int ran, THREADED)
{
  ...
  srandomBit_th((MYTHREAD+17)*53, TH);
  
  switch(ran)
    {
    case PSEUDORAN:
      return randomBit_th(TH);
      break;
    case ALLMULT:
      return 1;
      break;
